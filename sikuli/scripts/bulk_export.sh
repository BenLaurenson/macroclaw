#!/usr/bin/env bash
# bulk_export.sh -- Wrapper to run the SikuliX bulk/historical export automation.
#
# Run manually or on a schedule for full data backups.
# Locates the SikuliX JAR and executes bulk_export.py as a Jython script.
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting bulk export..."

"$JAVA_BIN" -jar "$SIKULIX_JAR" \
    -r "$SCRIPT_DIR/bulk_export.py" \
    -- "$CONFIG_FILE"

EXIT_CODE=$?

if [[ $EXIT_CODE -eq 0 ]]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Bulk export completed successfully."
else
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Bulk export FAILED (exit code: $EXIT_CODE)." >&2
fi
